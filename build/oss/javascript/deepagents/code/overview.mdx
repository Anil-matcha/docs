...
<Steps>
    ...
    <Step title="Install and launch" icon="terminal">
        OpenAI, Anthropic, and Google are installed by default. Other providers (Ollama, Groq, xAI, etc.) are available as optional extras—see [Providers](/oss/javascript/deepagents/code/providers) for details.
    ...
    <Step title="Add provider credentials" icon="key">
        Deep Agents Code works with any LLM that supports tool calling. Use the `/auth` command to set an API key for your chosen providers — see [Provider credentials](/oss/javascript/deepagents/code/configuration#provider-credentials) for the full flow and storage details.

        For additional providers and headless runs, see [Providers](/oss/javascript/deepagents/code/providers).

        <Note>
            Web search uses [Tavily](https://tavily.com) and is **not** configured through `/auth`. If you see "Web search disabled — `TAVILY_API_KEY` is not set" on startup, add `TAVILY_API_KEY=tvly-...` to `~/.deepagents/.env` and run `/reload` (or restart). See [Enable web search with Tavily](/oss/javascript/deepagents/code/configuration#enable-web-search-with-tavily).
        </Note>
    </Step>
...
* <Icon icon="cloud" size={16} /> **[Remote sandboxes](/oss/javascript/deepagents/code/remote-sandboxes)** - run agent tools in LangSmith, Daytona, Modal, Runloop, or AgentCore instead of your local machine. The linked page covers provider installation, credentials, sandbox flags (`--sandbox`, `--sandbox-id`, `--sandbox-setup`), and setup scripts.
* <Icon icon="search" size={16} /> **Web search** - search the web for up-to-date information and documentation. Requires a Tavily API key in [`TAVILY_API_KEY`](/oss/javascript/deepagents/code/configuration#enable-web-search-with-tavily).
...
* <Icon icon="users" size={16} /> **[Subagents](/oss/javascript/deepagents/code/subagents)** - delegate work with the `task` tool. In Deep Agents Code, define custom subagents as `AGENTS.md` files; the linked page covers paths, frontmatter, and examples.
...
* <Icon icon="code" size={16} /> **[JavaScript interpreter](/oss/javascript/deepagents/code/interpreter)** - opt-in `js_eval` REPL backed by QuickJS, with an optional programmatic tool-calling allowlist for host tools. Local mode only; requires the `quickjs` extra.
...
<Accordion title="Full list of built-in tools">
    ...
    | `web_search` | Search the web using Tavily (requires `TAVILY_API_KEY` — see [Enable web search](/oss/javascript/deepagents/code/configuration#enable-web-search-with-tavily)) | Required<sup>1</sup> |
    ...
    | `js_eval` | Run JavaScript in a persistent QuickJS REPL with optional programmatic tool calling. Available only when launched with `--interpreter` (see [Interpreter](/oss/javascript/deepagents/code/interpreter)) | - |
...
<AccordionGroup>
    <Accordion title="Command-line options" icon="flag">
        ...
        | `--interpreter`        | Enable the `js_eval` JavaScript interpreter middleware on the main agent. Local mode only; requires the `quickjs` extra. See [Interpreter](/oss/javascript/deepagents/code/interpreter) |
    ...
    <Accordion title="CLI commands" icon="terminal">
        ...
        | `dcode mcp login NAME [--config PATH]` | Run the OAuth login flow for an MCP server marked `auth: "oauth"`. See [MCP tools](/oss/javascript/deepagents/code/mcp-tools#oauth-login) |
...
<AccordionGroup>
    <Accordion title="Slash commands" icon="slash">
        ...
        - `/auth` - Manage stored API keys for model providers. See [Provider credentials](/oss/javascript/deepagents/code/configuration#provider-credentials) for details

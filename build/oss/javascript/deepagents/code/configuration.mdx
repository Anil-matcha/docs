...
<ResponseField name="models" type="string[]" post={["optional"]}>
    ...
    Models listed here **bypass** the profile-based [filtering criteria](/oss/javascript/deepagents/code/providers#which-models-appear-in-the-switcher) and always appear in the switcher. This makes it the recommended way to surface models that are excluded because their profile lacks `tool_calling` support or doesn't exist yet.
...
<ResponseField name="api_key_env" type="string" post={["optional"]}>
    The **name** of the environment variable that holds the API key (e.g., `"OPENAI_API_KEY"`), not the key itself. Deep Agents Code reads the credential from this env var at startup to verify access before creating the model. Most chat model packages read from a default env var automatically. See the [Provider reference](/oss/javascript/deepagents/code/providers#provider-reference) table for which variable each provider checks.
</ResponseField>
...
Set `[interpreter]` keys in `~/.deepagents/config.toml` to enable the `js_eval` JavaScript interpreter middleware on every session and configure its runtime limits and PTC allowlist:
...
The CLI flags `--interpreter` and `--interpreter-tools` override the matching keys for a single session. The interpreter is local-mode only and requires the `quickjs` optional extra.

For the full configuration reference, the `safe` / `all` PTC presets, and the HITL trade-off, see [Interpreter](/oss/javascript/deepagents/code/interpreter).

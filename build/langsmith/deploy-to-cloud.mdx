...
You can also deploy by building a Docker image locally, pushing it to a container registry, and providing the image URL to the control plane UI. The workflow is:

1. Test your application locally using `langgraph dev` or [Studio](/langsmith/studio). For a full guide, refer to the [Local server quickstart](/langsmith/local-dev-testing).
2. Build a Docker image of your application using the [`langgraph build`](/langsmith/cli#build) command:
...
3. Push your image to a container registry accessible by your infrastructure.

    <Tip>
    Tag your images with version information (e.g., `my-registry.com/my-app:v1.0.0`) to make rollbacks easier.
    </Tip>

4. Deploy from the [control plane UI](/langsmith/control-plane#control-plane-ui):

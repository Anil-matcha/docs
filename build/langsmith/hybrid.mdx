...
- **LangSmith SaaS**: Omit `LANGSMITH_ENDPOINT` (or set it to the LangSmith SaaS endpoint). Traces are sent to LangChain's managed LangSmith.
...
<Note>
Looking for the previous hybrid model with a LangChain-managed control plane orchestrating Agent Servers in your infrastructure? See [Setup guide (legacy)](/langsmith/hybrid-legacy).
</Note>

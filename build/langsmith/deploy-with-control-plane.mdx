...
- **[Hybrid (legacy) setup](/langsmith/hybrid-legacy)**: For the legacy hybrid model with a LangChain-managed control plane.
...
Applications deployed to self-hosted LangSmith instances with control plane use Docker images. In this guide, the application deployment workflow is:
...
- Completed infrastructure setup to enable your [data plane](/langsmith/data-plane) to receive application deployments:
  - [Hybrid (legacy) setup](/langsmith/hybrid-legacy): Installs data plane components (listener, operator, CRDs) in your Kubernetes cluster that connect to LangChain's managed control plane.

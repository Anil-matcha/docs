...
## Deploy applications with the control plane

Once LangSmith Deployment is enabled, deploy your applications by building a Docker image locally, pushing it to a container registry accessible by your Kubernetes cluster, and deploying via the control plane UI.

### Prerequisites

- LangSmith Deployment enabled on your self-hosted instance (the previous sections of this guide).
- Access to the [LangSmith UI](https://smith.langchain.com?utm_source=docs&utm_medium=cta&utm_campaign=langsmith-signup&utm_content=langsmith-deploy-self-hosted-full-platform) with LangSmith Deployment enabled.
- A container registry accessible by your Kubernetes cluster. If using a private registry that requires authentication, you must configure image pull secrets — see [Private registry authentication](#private-registry-authentication) below.
...
For a full guide on local testing, refer to the [Local server quickstart](/langsmith/local-dev-testing).
...
The [control plane UI](/langsmith/control-plane#control-plane-ui) allows you to create and manage deployments, view logs and metrics, and update configurations. To create a new deployment in the [LangSmith UI](https://smith.langchain.com?utm_source=docs&utm_medium=cta&utm_campaign=langsmith-signup&utm_content=langsmith-deploy-self-hosted-full-platform):
...
Configure `imagePullSecrets` in your LangSmith Helm chart's `values.yaml` file. For detailed steps on creating image pull secrets for different registry providers, refer to the [Kubernetes documentation on pulling images from private registries](https://kubernetes.io/docs/tasks/configure-pod-container/pull-image-private-registry/).

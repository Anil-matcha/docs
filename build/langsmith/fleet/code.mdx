...
description: Invoke Fleet agents via the LangGraph SDK or REST API, or download and run them locally with the fleet-deepagent-export package.
...
- **[Export to code](#export-to-code)** — download your agent's configuration and run it locally as a self-contained Python project using the `fleet-deepagent-export` package.
...
The [`fleet-deepagent-export`](https://pypi.org/project/fleet-deepagent-export/) package ([GitHub](https://github.com/langchain-ai/fleet-deepagent-export)) handles reading the exported configuration and wiring up your agent with MCP tools, subagents, and skills.
...
The starter project at [`examples/template-agent/`](https://github.com/langchain-ai/fleet-deepagent-export/tree/main/examples/template-agent) is the recommended starting point. Clone the repo and copy the starter:

```bash
git clone https://github.com/langchain-ai/fleet-deepagent-export.git
cp -R fleet-deepagent-export/examples/template-agent my-agent
cd my-agent
```

### 2. Export your agent from Fleet

In the [LangSmith UI](https://smith.langchain.com?utm_source=docs&utm_medium=cta&utm_campaign=langsmith-signup&utm_content=langsmith-fleet-code), open your agent and export it as a `.zip` file. Then drop the contents into the `fleet/` directory of your starter project:
...
The starter ships with `langchain-anthropic`, `langchain-openai`, and `langchain-google-genai`. For any other provider (e.g. `bedrock:`, `mistralai:`), add the matching `langchain-<provider>` package to `pyproject.toml`.
